...
class A2CNet(nn.Module):
    ...
    def forward(self, x):
        fx = x.float()/ 256
        conv_out = self.conv(fx).view(fx.size()[0], -1)# only keep batch level
        fc_out = self.fc(conv_out)
        return self.policy(fc_out), self.value(fc_out)


class EnvironModel(nn.Module):
    def __init__(self, input_shape,n_actions):
        super(EnvironModel, self).__init__()
        ...
        self.deconv = nn.ConvTranspose2d(64, 1, 4, 4, 0)
        ...
        reward_conv_out = self._get_reward_conv_out_size((n_planes, ) + input_shape[1:])
    ...
    def _get_reward_conv_out_size(self, shape):
