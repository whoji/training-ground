# based on https://github.com/orrivlin/Navigation-HER/blob/master/dqn_HER.py
...
class AgentDQN:
    def __init__(self, env, gamma, buffer_size, ddqn):
        ...
        self.her = HER()
        self.gamma = gamma
        self.opt = opt.Adam(self.model.parameters(),lr=0.0001)
        self.batch_size = 16
        self.epsilon = 0.1
    ...
    def run_episode(self):
        ...
        for t in range(max_t):
            ...
            #[dc(state.squeeze(0).numpy()),dc(action),dc(reward),dc(new_state.squeeze(0).numpy()),dc(done)]
            data = Data(s=state.squeeze(0).numpy(),
                        a=action,
                        r=reward,
                        s_new=new_state.squeeze().numpy()
                        terminal=done)
            self.replay_buffer.append(data)
            self.her.keep(deepcopy(data))
